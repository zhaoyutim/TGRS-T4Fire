...
def get_convlstm_unet(input_shape):
    inputs = Input(input_shape)
    conv1 = ConvLSTM2D(64, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal', return_sequences=True)(inputs)
    # conv1 = ConvLSTM2D(64, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal', return_sequences=True)(conv1)
    ...
    conv4 = ConvLSTM2D(512, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal', return_sequences=True)(pool3)
    # conv4 = ConvLSTM2D(512, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal', return_sequences=True)(conv4)
    drop4 = TimeDistributed(Dropout(0.5))(conv4)
    pool4 = TimeDistributed(MaxPooling2D(pool_size=(2, 2)))(drop4)

    conv5 = ConvLSTM2D(1024, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal', return_sequences=True)(pool4)
    # conv5 = ConvLSTM2D(1024, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal', return_sequences=True)(conv5)
    ...
    merge6 = tf.concat([drop4,up6], axis = 4)
    conv6 = TimeDistributed(Conv2D(512, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal'))(merge6)
